-- Migration: Add Google LLM provider and insert all llm_models data
-- Date: 2025-09-03
-- Description: Add missing Google LLM provider and insert llm_models data

BEGIN;

-- First, add the missing Google LLM provider
INSERT INTO client_mgmt.llms (id, provider, api_key_env_var, base_url, is_active, created_at, updated_at)
VALUES (4, 'google', 'GOOGLE_API_KEY', NULL, true, NOW(), NOW())
...
SELECT id, provider, is_active FROM client_mgmt.llms ORDER BY id;
