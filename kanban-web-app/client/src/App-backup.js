...
function App() {
  ...
  const onDragEnd = async (result) => {
    ...
    if (startColumn === finishColumn) {
      ...
      if (currentGame) {
        const moveNotation = `${draggableId}:${source.droppableId.toUpperCase()}→${destination.droppableId.toUpperCase()}`;
        const moveDescription = `Moving ${draggableId} from ${startColumn.title} to ${finishColumn.title}`;
        const boardResponse = `${draggableId} moved to ${finishColumn.title} - monitoring progress and dependencies`;

        try {
          await axios.post(`${API_BASE}/games/${currentGame.game_id}/moves`, {
            moveNotation,
            moveDescription,
            boardResponse
          });
