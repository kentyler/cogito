...
async function updateConflictClubPatterns() {
    ...
    try {
        ...
        const karlId = await findParticipantId(db, 'Karl Perry');
        if (karlId) {
        ...
        const lindenId = await findParticipantId(db, 'Linden Vazey');
        if (lindenId) {
        ...
        const ianId = await findParticipantId(db, 'Ian Palonis');
        if (ianId) {
        ...
        const julianId = await findParticipantId(db, 'Julian Andrews');
        if (julianId) {
        ...
        const brunoId = await findParticipantId(db, 'Bruno Guardia');
        if (brunoId) {
        ...
        const kenId = await findParticipantId(db, 'Kenneth Tyler');
        if (kenId) {
...
async function findParticipantId(db, name) {
    const result = await db.pool.query("SELECT find_participant_id($1)", [name]);
    return result.rows[0].find_participant_id;
}
