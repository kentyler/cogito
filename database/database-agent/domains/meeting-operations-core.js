...
export class MeetingOperationsCore {
  ...
  async create(meetingData) {
    ...
    const query = `
      INSERT INTO meetings.meetings (
        ...
        metadata, recall_bot_id, meeting_url, status, full_transcript, created_at, updated_at
      )
    ...
    const result = await this.connector.query(query, [
      ...
      full_transcript ? JSON.stringify(full_transcript) : null
    ]);
    return result.rows[0];
  }
  ...
  async delete(meetingId) {
    return await this.connector.transaction(async (client) => {
      ...
      if (this.turnsOperations) {
        ...
        const turns = await this.turnsOperations.getByMeetingId(meetingId);
        deletedTurnsCount = turns.length;
        
        // Delete all turns for this meeting using turns domain
        if (deletedTurnsCount > 0) {
          await this.turnsOperations.deleteByMeetingId(meetingId);
        }
      } else {
  ...
  /**
   * Get meeting client info
   * @param {string} meetingId - Meeting ID
   * @returns {Object|null} Client info for the meeting
   */
  async getMeetingClientInfo(meetingId) {
    const query = `
      ...
      LEFT JOIN client_mgmt.clients c ON m.client_id = c.id
      WHERE m.id = $1
    `;
    const result = await this.connector.query(query, [meetingId]);
    return result.rows[0] || null;
  ...
  async getMeetingClientId(meetingId) {
    ...
    `;
    const result = await this.connector.query(query, [meetingId]);
    return result.rows[0] ? result.rows[0].client_id : null;
  }
}