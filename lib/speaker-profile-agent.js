...
export class SpeakerProfileAgent {
   ...
   * @param {string} blockId - Current meeting block ID
   */
  async generateProfile(user, blockId) {
    console.log(`[SpeakerProfile] Generating profile for user_id: ${user.user_id}`);

    // Get user's last N turns across all conversations
    const turnsQuery = `
      SELECT t.content, t.timestamp, t.metadata, b.name as block_name
      FROM conversation.turns t
      JOIN conversation.blocks b ON t.block_id = b.block_id
      WHERE t.user_id = $1 
    ...
    await this.storeProfileAsFile(user, profile, blockId);
  }
...
${turns.slice(0, 10).map((turn, i) => 
  `**${new Date(turn.timestamp).toLocaleDateString()}** in *${turn.block_name}*:\n"${turn.content.substring(0, 200)}${turn.content.length > 200 ? '...' : ''}"\n`
).join('\n')}
...
`;
   ...
   */
  async storeProfileAsFile(user, profileContent, blockId) {
    ...
    try {
      ...
      // Link to the current meeting block
      await this.databaseAgent.query(
        `INSERT INTO conversation.block_meeting_files (block_meeting_id, file_upload_id, created_by_user_id) 
         SELECT bm.id, $1, $2 FROM conversation.block_meetings bm 
         JOIN conversation.blocks b ON bm.block_id = b.block_id 
         WHERE b.block_id = $3`,
        [fileUpload.id, user.user_id, blockId]
      );
      
      console.log(`[SpeakerProfile] Stored profile for ${user.alias || user.email} as file: ${filename}`);
