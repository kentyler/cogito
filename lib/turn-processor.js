...
export class TurnProcessor {
  ...
  async findSimilarTurns(turnId, limit = 10, minSimilarity = 0.7, parentClientId = null) {
    // Build client filter and parameters for mini-horde support
    let clientFilter, queryParams;
    if (parentClientId) {
      // Mini-horde: search both own client AND parent client data
      clientFilter = `AND (m.client_id = tm.client_id OR m.client_id = $4)`;
      queryParams = [turnId, limit, minSimilarity, parentClientId];
    } else {
      // Regular client: search only own client data
      clientFilter = `AND m.client_id = tm.client_id`;
      queryParams = [turnId, limit, minSimilarity];
    }
    ...
    const query = `
      ...
      SELECT 
        ...
        m.client_id,
        CASE 
          WHEN m.client_id = (SELECT client_id FROM target_meeting) THEN 'own'
          ELSE 'parent'
        END as data_source,
      ...
      ${clientFilter}  -- Support both own and parent client data
