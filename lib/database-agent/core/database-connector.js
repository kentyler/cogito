...
export class DatabaseConnector {
  ...
  async connect() {
    ...
    try {
      ...
      // Log connection errors if event logger is available
      if (this.eventLogger) {
        await this.eventLogger.logError('database_connection_failed', error, {
          severity: 'critical',
  ...
  async query(sql, params = []) {
    ...
    try {
      ...
      // Log query errors
      if (this.eventLogger) {
        await this.eventLogger.logError('database_query_error', error, {
          sql: sql.substring(0, 1000), // Limit SQL length in logs
  ...
  async transaction(callback) {
    ...
    try {
      ...
      if (this.eventLogger) {
        await this.eventLogger.logError('database_transaction_error', error, {
          component: 'DatabaseConnector',
          severity: 'error',
