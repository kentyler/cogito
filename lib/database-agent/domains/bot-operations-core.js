...
export class BotOperationsCore {
  ...
  async setBotStatusLeaving(botId) {
    // Try first with recall_bot_id, then with meeting id if that fails
    let query = `
      UPDATE meetings.meetings 
    ...
    let result = await this.connector.query(query, [botId]);
    
    if (result.rows.length === 0) {
      // If no match with recall_bot_id, try with meeting id
      query = `
        UPDATE meetings.meetings 
        SET status = 'leaving', updated_at = NOW()
        WHERE id = $1
        RETURNING *
      `;
      result = await this.connector.query(query, [botId]);
    }
