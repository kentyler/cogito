...
export class ClientOperationsCore {
  ...
  async createClient(clientData) {
    const { name, story = null, metadata = {}, parent_client_id = null } = clientData;
    ...
    try {
      ...
      const result = await this.connector.query(query, [name, story, metadata, parent_client_id]);
  ...
  async getClientName(clientId) {
    const query = `SELECT name FROM client_mgmt.clients WHERE id = $1`;
    const result = await this.connector.query(query, [clientId]);
