 ...
 * with sequential block_index numbers for ordering.
 * 
 * Flush conditions:
...
export class TranscriptBufferAgent {
  ...
  /**
   * Initialize for a new meeting/block
   */
  startNewBlock({ blockId, clientId }) {
    ...
    console.log(`[TranscriptBuffer] Started new block: ${blockId}`);
  }
  ...
  async addChunk({ speaker, text, timestamp }) {
    if (!this.blockId) {
      throw new Error('Must call startNewBlock before adding chunks');
    }
  ...
  async flush() {
    ...
    console.log(`[TranscriptBuffer] Flushing turn ${this.blockIndex} for speaker: ${this.currentSpeaker} (${turn.content.length} chars)`);
  ...
  /**
   * Force flush any remaining content and end the block
   */
  async endBlock() {
    await this.flush();
    ...
    const summary = {
      blockId: this.blockId,
      totalTurns: this.blockIndex,
      lastSpeaker: this.currentSpeaker
    };
    
    console.log(`[TranscriptBuffer] Ended block ${this.blockId} with ${this.blockIndex} turns`);
